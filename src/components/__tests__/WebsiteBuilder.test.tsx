...
const createWebsiteBuilder = (
  ...
  blocks: [
    ...
    createBlock({
      ...
      body: "Share your timeline and goals so we can suggest the best launch setup.",
...
describe("WebsiteBuilder", () => {
  ...
  it("toggles to preview mode and renders hero, features, cta, text, and lead capture sections", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Talk to a specialist" })).toBeInTheDocument();
  });
  ...
  it.each([
    ...
    ["Add Lead Capture", "lead_capture", "Get qualified enquiries in your inbox"],
  ...
  it("renders lead capture preview as a non-functional enquiry form mock", async () => {
    const user = userEvent.setup();
    ...
    expect(screen.getByRole("heading", { name: "Talk to a specialist" })).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: "Lead capture email" })).toBeDisabled();
    expect(screen.getByRole("textbox", { name: "Lead capture website" })).toBeDisabled();
    expect(screen.getByRole("textbox", { name: "Lead capture enquiry" })).toBeDisabled();
